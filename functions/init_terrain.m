function [terrain,egv,ns] = init_terrain(filenames,egv,ns)
...
if ischar(egv.x.xN)
    if strcmp(egv.x.xN,'last')
        ns.N = round((terrain.info(end,1)+(egv.x.step-1)/2)/egv.x.step);
    else
...
if ischar(egv.x.xN)
    ...
    terrain.dist(ns.N+1) = terrain.info(end,1);
    terrain.alti(ns.N+1) = terrain.info(end,2);
else
...
%set final distance of the egv to the final distance of the terrian profile
egv.x.xN = terrain.dist(end);
