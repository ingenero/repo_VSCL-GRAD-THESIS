...
pre.v_in  = [25 25]; %[km/h] velocity profile
pre.x_in  = [25 3000]; %[m] position where velocity takes place

%--TRAFFIC LIGHT (light)
light.exist  = 'n'; %are there any stop lights? (y/n)
light.pos    = [750 2250]; %[m] position of each traffic light
light.intype = {'auto','auto'};
% light.pos    = 2250; %[m] position of each traffic light
% light.intype = {'auto'};
light.green  = 100; %[s] duration of green (for 'auto' type)
light.red    = 60; %[s] duration of red (for 'auto' type)
...
while ns.k >= 1
    ...
    waitbar(percent_done/100,h_wb,...
        ['Calculating Optimal Trajectory... (' num2str(ceil(percent_done)) '%)']);
